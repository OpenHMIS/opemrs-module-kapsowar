 ...
 */
package org.openmrs.module.kapsowar.web.controller;
 ...
 */
@Controller(value = "kapsowarJasperReportController")
@RequestMapping(value = KapsowarConstants.JASPER_REPORT_PAGE)
public class JasperReportController extends ReportsControllerBase {
	...
	public String parse(int reportId, WebRequest request, HttpServletResponse response) throws IOException {
		...
		if (kapsowarSettings.getDefaultShiftReportId() != null && reportId == kapsowarSettings.getDefaultShiftReportId()) {
			return renderShiftReport(reportId, request, response);
		} else if (kapsowarSettings.getDepartmentCollectionsReportId() != null
		        && reportId == kapsowarSettings.getDepartmentCollectionsReportId()) {
			return renderDateRangeReport(reportId, "Department Collections", request, response);
		} else if (kapsowarSettings.getDepartmentRevenueReportId() != null
		        && reportId == kapsowarSettings.getDepartmentRevenueReportId()) {
			return renderDateRangeReport(reportId, "Department Revenue", request, response);
		} else if (kapsowarSettings.getDailyShiftSummaryReportId() != null
		        && reportId == kapsowarSettings.getDailyShiftSummaryReportId()) {
			return renderDateRangeReport(reportId, "Daily Shift Summary", request, response);
		} else if (kapsowarSettings.getShiftSummaryReportId() != null
		        && reportId == kapsowarSettings.getShiftSummaryReportId()) {
			return renderDateRangeReport(reportId, "Shift Summary", request, response);
		} else if (kapsowarSettings.getCashierBillAdjustmentsId() != null && reportId == kapsowarSettings
		        .getCashierBillAdjustmentsId()) {
			return renderDateRangeReport(reportId, "Cashier Bill Adjustments", request, response);
